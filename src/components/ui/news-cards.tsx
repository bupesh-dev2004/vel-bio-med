          ...
          {newsCards.map((card) => {
            ...
            return (
              <motion.article
                ...
                className="bg-card border border-border/50 rounded-lg overflow-hidden transition-all duration-300 cursor-pointer group flex flex-col h-full"
        ...
        <AnimatePresence>
          {selectedCard && (
            <>
              ...
              <div className="fixed inset-0 flex items-center justify-center p-4 md:p-8 lg:p-16 z-55 overflow-y-auto">
                <motion.div
                  layoutId={`card-${selectedCard.id}`}
                  className="w-full max-w-4xl bg-card border border-border rounded-xl overflow-hidden shadow-2xl relative max-h-[90vh] flex flex-col"
                >
                  {/* Close Button */}
                  <motion.button
                    className="absolute top-4 right-4 w-8 h-8 bg-background/80 hover:bg-background border border-border rounded-full flex items-center justify-center z-20 cursor-pointer shadow-sm"
